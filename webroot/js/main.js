  ...
  var MAIN = class {
    ...
    onOutsideClick(selector, callback) {
      document.addEventListener("click", (event2) => {
        var target = event2.target.parentNode.closest(selector);
        if (target == null && document.querySelector(selector) != event2.target) {
          callback(event2, target);
        }
      });
    }
  };
